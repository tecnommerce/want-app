...
function renderizarPedidosDesktop() {
    ...
    for (const p of pedidosFiltrados) {
        ...
        } else { productosResumen = 'Sin productos'; }
        const total = formatearPrecio(p.total || 0);
        let botonesHTML = '';
        if (estado === 'preparando') {
            botonesHTML = `<button class="btn-tabla btn-whatsapp" onclick="confirmarPedidoWhatsApp(${p.id}, this)"><i class="fab fa-whatsapp"></i> Confirmar</button><button class="btn-tabla btn-preparar" onclick="actualizarEstado(${p.id}, 'en preparacion', this)"><i class="fas fa-utensils"></i> Preparar</button><button class="btn-tabla btn-editar" onclick="abrirModalEditarPedido(${p.id})"><i class="fas fa-edit"></i></button><button class="btn-tabla btn-cancelar-tabla" onclick="cancelarPedido(${p.id}, this)"><i class="fas fa-trash-alt"></i></button>`;
        } else if (estado === 'en preparacion') {
            botonesHTML = `<button class="btn-tabla btn-pedido-listo" onclick="abrirModalAsignarDelivery(${p.id})"><i class="fas fa-check-circle"></i> Listo</button><button class="btn-tabla btn-editar" onclick="abrirModalEditarPedido(${p.id})"><i class="fas fa-edit"></i></button><button class="btn-tabla btn-cancelar-tabla" onclick="cancelarPedido(${p.id}, this)"><i class="fas fa-trash-alt"></i></button>`;
        } else if (estado === 'en camino') {
            botonesHTML = `<button class="btn-tabla btn-notificar" onclick="notificarEnCamino(${p.id}, this)"><i class="fab fa-whatsapp"></i> Notificar</button><button class="btn-tabla btn-entregar" onclick="actualizarEstado(${p.id}, 'entregado', this)"><i class="fas fa-check-double"></i> Entregar</button><button class="btn-tabla btn-editar" onclick="abrirModalEditarPedido(${p.id})"><i class="fas fa-edit"></i></button><button class="btn-tabla btn-cancelar-tabla" onclick="cancelarPedido(${p.id}, this)"><i class="fas fa-trash-alt"></i></button>`;
        } else if (estado === 'entregado') {
            botonesHTML = `<button class="btn-tabla btn-editar" onclick="abrirModalEditarPedido(${p.id})"><i class="fas fa-edit"></i></button><button class="btn-tabla btn-cancelar-tabla" onclick="cancelarPedido(${p.id}, this)"><i class="fas fa-trash-alt"></i></button>`;
        }
        html += `<tr onclick="verPedidoCompleto(${p.id})" style="cursor: pointer;"><td class="col-id">#${numeroMostrar}</td><td class="col-fecha">${fecha.toLocaleString('es-AR')}</td><td class="col-cliente">${escapeHTML(p.cliente_nombre || 'Sin nombre')}</td><td class="col-telefono">${p.cliente_telefono || '-'}</td><td class="col-direccion">${escapeHTML(p.direccion || '-')}</td><td class="col-pago">${metodoPago}</td><td class="col-productos"><span class="productos-preview">${escapeHTML(productosResumen)}</span></td><td class="col-total">${total}</td><td class="col-estado"><span class="estado-badge estado-${estado.replace(' ', '-')}">${getEstadoTexto(estado)}</span></td><td class="col-acciones">${botonesHTML}</td></tr>`;
    }
    html += `</tbody></table>`;
...
function renderizarPedidosMovil() {
    ...
    for (const p of pedidosFiltrados) {
        ...
        html += `<div class="pedido-card-movil" data-pedido-id="${p.id}"><div class="pedido-card-header"><span class="pedido-numero-movil">#${numeroMostrar}</span><span class="pedido-estado-movil ${estadoClase}">${estadoTexto}</span></div><div class="pedido-card-body"><div class="pedido-cliente-movil">${escapeHTML(p.cliente_nombre || 'Sin nombre')}</div><div class="pedido-resumen-movil">📦 ${escapeHTML(productosResumen)}</div><div class="pedido-resumen-movil">💰 ${formatearPrecio(p.total || 0)}</div></div><div class="pedido-card-footer"><button class="btn-ver-pedido-movil" onclick="verPedidoCompletoMovil(${p.id})"><i class="fas fa-eye"></i> Ver pedido</button></div></div>`;
    }
    html += `</div>`;
    container.innerHTML = html;
}
...
function verPedidoCompletoMovil(pedidoId) {
    ...
    if (pedido.productos && Array.isArray(pedido.productos) && pedido.productos.length > 0) {
        pedido.productos.forEach(pr => { productosHTML += `<div class="producto-detalle"><span>${pr.cantidad}x ${escapeHTML(pr.nombre)}</span><span>${formatearPrecio(pr.precio * pr.cantidad)}</span></div>`; });
    } else { productosHTML = '<p>No hay productos</p>'; }
    let detallesHTML = '';
    if (pedido.detalles && pedido.detalles.trim()) { detallesHTML = `<div class="detalle-seccion"><strong><i class="fas fa-pen"></i> Detalles:</strong><p>${escapeHTML(pedido.detalles)}</p></div>`; }
    let botonesAccion = '';
    if (estado === 'preparando') {
        botonesAccion = `<button class="btn-tabla btn-whatsapp" onclick="confirmarPedidoWhatsApp(${pedido.id}, this)"><i class="fab fa-whatsapp"></i> Confirmar</button><button class="btn-tabla btn-preparar" onclick="actualizarEstado(${pedido.id}, 'en preparacion', this)"><i class="fas fa-utensils"></i> Preparar</button><button class="btn-tabla btn-editar" onclick="abrirModalEditarPedido(${pedido.id})"><i class="fas fa-edit"></i> Editar</button><button class="btn-tabla btn-cancelar-tabla" onclick="cancelarPedido(${pedido.id}, this)"><i class="fas fa-trash-alt"></i> Cancelar</button>`;
    } else if (estado === 'en preparacion') {
        botonesAccion = `<button class="btn-tabla btn-pedido-listo" onclick="abrirModalAsignarDelivery(${pedido.id})"><i class="fas fa-check-circle"></i> Listo</button><button class="btn-tabla btn-editar" onclick="abrirModalEditarPedido(${pedido.id})"><i class="fas fa-edit"></i> Editar</button><button class="btn-tabla btn-cancelar-tabla" onclick="cancelarPedido(${pedido.id}, this)"><i class="fas fa-trash-alt"></i> Cancelar</button>`;
    } else if (estado === 'en camino') {
        botonesAccion = `<button class="btn-tabla btn-notificar" onclick="notificarEnCamino(${pedido.id}, this)"><i class="fab fa-whatsapp"></i> Notificar</button><button class="btn-tabla btn-entregar" onclick="actualizarEstado(${pedido.id}, 'entregado', this)"><i class="fas fa-check-double"></i> Entregar</button><button class="btn-tabla btn-editar" onclick="abrirModalEditarPedido(${pedido.id})"><i class="fas fa-edit"></i> Editar</button><button class="btn-tabla btn-cancelar-tabla" onclick="cancelarPedido(${pedido.id}, this)"><i class="fas fa-trash-alt"></i> Cancelar</button>`;
    } else if (estado === 'entregado') {
        botonesAccion = `<button class="btn-tabla btn-editar" onclick="abrirModalEditarPedido(${pedido.id})"><i class="fas fa-edit"></i> Editar</button><button class="btn-tabla btn-cancelar-tabla" onclick="cancelarPedido(${pedido.id}, this)"><i class="fas fa-trash-alt"></i> Cancelar</button>`;
    }
    const modalContent = `<div class="modal" id="modal-pedido-completo-movil" style="display: flex;"><div class="modal-content" style="max-width: 550px;"><div class="modal-header"><h3><i class="fas fa-receipt"></i> Pedido #${numeroMostrar}</h3><button class="modal-close" onclick="cerrarModalPedidoCompletoMovil()">&times;</button></div><div class="modal-body"><div class="detalle-seccion"><strong><i class="fas fa-calendar"></i> Fecha:</strong><p>${fecha.toLocaleString('es-AR')}</p></div><div class="detalle-seccion"><strong><i class="fas fa-user"></i> Cliente:</strong><p>${escapeHTML(pedido.cliente_nombre || 'Sin nombre')}</p></div><div class="detalle-seccion"><strong><i class="fas fa-phone"></i> Teléfono:</strong><p>${pedido.cliente_telefono || 'Sin teléfono'}</p></div><div class="detalle-seccion"><strong><i class="fas fa-map-marker-alt"></i> Dirección:</strong><p>${escapeHTML(pedido.direccion || 'Sin dirección')}</p></div><div class="detalle-seccion"><strong><i class="fas fa-money-bill-wave"></i> Pago:</strong><p>${metodoPago}</p></div><div class="detalle-seccion"><strong><i class="fas fa-box"></i> Productos:</strong><div class="productos-detalle">${productosHTML}</div></div>${detallesHTML}<div class="detalle-seccion total"><strong><i class="fas fa-calculator"></i> Total:</strong><p class="total-monto">${formatearPrecio(pedido.total || 0)}</p></div></div><div class="modal-footer" style="flex-wrap: wrap; gap: 8px;">${botonesAccion}</div></div></div>`;
...
function verPedidoCompleto(pedidoId) {
    ...
    let detallesHTML = '';
    if (pedido.detalles && pedido.detalles.trim()) { detallesHTML = `<div class="detalle-seccion"><strong><i class="fas fa-pen"></i> Detalles del pedido:</strong><p>${escapeHTML(pedido.detalles)}</p></div>`; }
    const modalContent = `<div class="modal" id="modal-pedido-completo" style="display: flex;"><div class="modal-content" style="max-width: 550px;"><div class="modal-header"><h3><i class="fas fa-receipt"></i> Pedido #${numeroMostrar}</h3><button class="modal-close" onclick="cerrarModalPedidoCompleto()">&times;</button></div><div class="modal-body"><div class="detalle-seccion"><strong><i class="fas fa-calendar"></i> Fecha:</strong><p>${fecha.toLocaleString('es-AR')}</p></div><div class="detalle-seccion"><strong><i class="fas fa-user"></i> Cliente:</strong><p>${escapeHTML(pedido.cliente_nombre || 'Sin nombre')}</p></div><div class="detalle-seccion"><strong><i class="fas fa-phone"></i> Teléfono:</strong><p>${pedido.cliente_telefono || 'Sin teléfono'}</p></div><div class="detalle-seccion"><strong><i class="fas fa-map-marker-alt"></i> Dirección:</strong><p>${escapeHTML(pedido.direccion || 'Sin dirección')}</p></div><div class="detalle-seccion"><strong><i class="fas fa-money-bill-wave"></i> Método de pago:</strong><p>${metodoPago}</p></div><div class="detalle-seccion"><strong><i class="fas fa-box"></i> Productos:</strong><div class="productos-detalle">${productosHTML}</div></div>${detallesHTML}<div class="detalle-seccion total"><strong><i class="fas fa-calculator"></i> Total:</strong><p class="total-monto">${formatearPrecio(pedido.total || 0)}</p></div></div><div class="modal-footer"><button class="btn-primary" onclick="cerrarModalPedidoCompleto()">Cerrar</button></div></div></div>`;
...
async function guardarProducto() {
    ...
    if (imagenFile) {
        ...
        if (!imagenUrl) { mostrarToast('Error al subir imagen', 'error'); return; }
    }
    const data = { vendedor_id: vendedorActual.id, nombre, descripcion, precio, disponible: disponible === 'SI' ? true : false };
...
function cargarPerfil() {
    ...
    const pdesc = document.getElementById('perfil-descripcion');
    ...
    if (pdesc) pdesc.value = vendedorActual.descripcion || '';
    ...
    if (rubrosContainer) {
        if (rubrosActuales.length === 0) { rubrosContainer.innerHTML = '<span class="rubro-placeholder">No hay rubros seleccionados</span>'; }
        else { rubrosContainer.innerHTML = rubrosActuales.map(r => `<span class="rubro-tag">${escapeHTML(r)}</span>`).join(''); }
    }
    ...
    if (btnEditarRubros) {
        btnEditarRubros.onclick = () => {
            abrirModalRubros(rubrosTempPerfil, (nuevosRubros) => {
                ...
                if (rubrosContainer) {
                    if (nuevosRubros.length === 0) { rubrosContainer.innerHTML = '<span class="rubro-placeholder">No hay rubros seleccionados</span>'; }
                    else { rubrosContainer.innerHTML = nuevosRubros.map(r => `<span class="rubro-tag">${escapeHTML(r)}</span>`).join(''); }
                }
            });
        };
    }
    ...
    // Inicializar el botón de guardar descripción cuando se abre el modal
...
async function registrarVendedorConLogo(nombre, email, telefono, direccion, horario, password, logoFile, rubros) {
    ...
    const response = await postAPI('registrarVendedor', { nombre, email, telefono, direccion, horario, password, logo_url: logoUrl, rubros });
...
function mostrarPanelRegistro() { 
    ...
    if (rubrosContainer) {
        rubrosContainer.innerHTML = RUBROS_DISPONIBLES.map(rubro => `<button type="button" class="btn-rubro-registro ${rubrosTempRegistro.includes(rubro) ? 'selected' : ''}" data-rubro="${rubro}">${rubro}</button>`).join('');
...
function actualizarListaRubrosRegistro() {
    ...
    if (listaSpan) {
        if (rubrosTempRegistro.length === 0) { listaSpan.textContent = 'Ninguno'; }
        else { listaSpan.textContent = rubrosTempRegistro.join(', '); }
    }
}
...
function actualizarReportes() {
    ...
    pedidos.forEach(pedido => {
        ...
        if (pedido.estado === 'entregado') {
            ...
            if (fechaPedido >= inicioSemana) ventasSemana += total;
            if (fechaPedido >= inicioMes) ventasMes += total;
        } else if (pedido.estado !== 'entregado' && pedido.estado !== 'cancelado') { pedidosPendientes++; }
    });
    ...
    pedidos.forEach(pedido => { if (pedido.productos && pedido.productos.length > 0) { pedido.productos.forEach(prod => { const nombre = prod.nombre; if (!ventasPorProducto[nombre]) ventasPorProducto[nombre] = 0; ventasPorProducto[nombre] += prod.cantidad; }); } });
    ...
    if (topProductosContainer) {
        if (topProductos.length === 0) { topProductosContainer.innerHTML = '<div class="loading-small">No hay productos vendidos</div>'; }
        else { topProductosContainer.innerHTML = topProductos.map(([nombre, cantidad]) => `<div class="reporte-item"><span class="reporte-item-nombre">${escapeHTML(nombre)}</span><span class="reporte-item-cantidad">${cantidad} unidades</span></div>`).join(''); }
    }
    ...
    pedidos.forEach(pedido => {
        if (pedido.estado === 'entregado') {
            ...
            if (diffDias >=0 && diffDias <7) { const diaIndex = fechaPedido.getDay(); ventasPorDia[diaIndex] += parseFloat(pedido.total) || 0; }
        }
    });
    const ventasDiasContainer = document.getElementById('reporte-ventas-dias');
    if (ventasDiasContainer) { ventasDiasContainer.innerHTML = diasSemana.map((dia,i)=>`<div class="dia-item"><span class="dia-nombre">${dia}</span><span class="dia-valor">${formatearPrecio(ventasPorDia[i])}</span></div>`).join(''); }
    ...
    if (ultimosPedidosContainer) {
        if (ultimosPedidos.length === 0) { ultimosPedidosContainer.innerHTML = '<div class="loading-small">No hay pedidos</div>'; }
        else { ultimosPedidosContainer.innerHTML = ultimosPedidos.map(p=>`<div class="pedido-item"><div class="pedido-info"><div class="pedido-numero">Pedido #${p.numero_orden || p.id}</div><div class="pedido-cliente">${escapeHTML(p.cliente_nombre || 'Sin nombre')}</div></div><div class="pedido-total">${formatearPrecio(p.total || 0)}</div></div>`).join(''); }
    }
}
...
async function iniciarPanel(vendedor) {
    ...
    if (headerLogoImg && vendedor.logo_url) { headerLogoImg.src = vendedor.logo_url; headerLogoImg.style.display = 'block'; }
...
function inicializarMenuAdmin() {
    ...
    function closeMenu() { if (menu) menu.classList.remove('active'); if (overlay) overlay.classList.remove('active'); document.body.style.overflow = ''; }
    function openMenu() { if (menu) menu.classList.add('active'); if (overlay) overlay.classList.add('active'); document.body.style.overflow = 'hidden'; }
...
async function cargarVendedorPorId(vendedorId) {
    try {
        ...
        if (response.success) {
            ...
            if (vendedor && vendedor.activo === true) { vendedorActual = vendedor; await iniciarPanel(vendedorActual); }
            else { cerrarSesion(); }
        }
...
function inicializarBuscador() {
    ...
    if (buscadorInput) { buscadorInput.addEventListener('input', (e) => { terminoBusqueda = e.target.value; renderizarPedidos(); }); }
    if (limpiarBtn) { limpiarBtn.addEventListener('click', () => { if (buscadorInput) { buscadorInput.value = ''; terminoBusqueda = ''; renderizarPedidos(); mostrarToast('Búsqueda limpiada', 'info'); } }); }
}

// ===================================================
// FUNCIÓN: GUARDAR SOLO LA DESCRIPCIÓN (VERSIÓN CORRECTA)
// ===================================================
async function guardarSoloDescripcion() {
    ...
    if (!vendedorId && vendedorActual) {
        ...
        console.log('✅ Vendedor ID obtenido de vendedorActual:', vendedorId);
    }
...
// ===================================================
// FUNCIÓN: INICIALIZAR BOTÓN DE DESCRIPCIÓN
// ===================================================
...
// ===================================================
// OBSERVAR MODAL DE PERFIL PARA INICIALIZAR BOTÓN
// ===================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('resize', () => { if (pedidos.length > 0) { renderizarPedidos(); } });
    ...
    if (loginForm) { loginForm.addEventListener('submit', async (e) => { e.preventDefault(); await login(); }); console.log('✅ Login form configurado'); }
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            if (response && response.success) { alert('Registro exitoso. Ahora podés iniciar sesión.'); mostrarPanelLogin(); registerForm.reset(); rubrosTempRegistro = []; const rubrosContainer = document.getElementById('rubros-selector-registro'); if (rubrosContainer) rubrosContainer.innerHTML = ''; document.getElementById('login-email').value = email; }
            else { alert(response?.error || 'Error al registrar'); }
        });
        const regLogo = document.getElementById('reg-logo');
        if (regLogo) { regLogo.addEventListener('change', (e) => { const file = e.target.files[0]; if (file) { const reader = new FileReader(); reader.onload = (ev) => { const preview = document.getElementById('reg-logo-preview'); if (preview) preview.innerHTML = `<img src="${ev.target.result}" style="max-width: 80px; border-radius: 12px;">`; }; reader.readAsDataURL(file); } }); }
    }
