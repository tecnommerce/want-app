        ...
        switch(action) {
            ...
            case 'getPedidos':
                ...
                for (const pedido of pedidos) {
                    const { data: prodPedido } = await supabase
            ...
            case 'registrarVendedor':
                const { data: authReg, error: authRegError } = await supabase.auth.signUp({
                    email: data.email,
                    password: data.password
                });
                if (authRegError) throw authRegError;
                
                const { data: newVendedor, error: vendRegError } = await supabase
                    .from('vendedores')
                    .insert([{
                        nombre: data.nombre,
                        email: data.email,
            ...
            case 'crearProducto':
                const { data: newProducto, error: prodCreateError } = await supabase
                    .from('productos')
                    .insert([{
            ...
            case 'crearPedido':
                ...
                const { data: pedidoNuevo, error: pedCreateError } = await supabase
                    .from('pedidos')
                    .insert([{
                        vendedor_id: data.vendedor_id,
            ...
            case 'crearDelivery':
                const { data: newDelivery, error: delCreateError } = await supabase
                    .from('delivery')
                    .insert([{
                        vendedor_id: data.vendedor_id,
                        nombre: data.nombre,
            ...
            case 'eliminarDelivery':
                const { error: delDeleteError } = await supabase
                    .from('delivery')
                    .update({ activo: false })
            ...
            case 'getBanners':
                let bannersQuery = supabase.from('banners').select('*').eq('activo', true).order('orden');
                if (data.vendedorId) {
            ...
            case 'getAllBanners':
                const { data: allBanners, error: allBanError } = await supabase
                    .from('banners')
                    .select('*')
                    .order('orden');
            ...
            case 'crearBanner':
                const { data: newBanner, error: banCreateError } = await supabase
                    .from('banners')
                    .insert([{
                        titulo: data.titulo,
                        imagen_url: data.imagen_url,
            ...
            case 'eliminarBanner':
                const { error: banDeleteError } = await supabase
                    .from('banners')
                    .delete()
                    .eq('id', data.bannerId);
...
// Función postAPI para compatibilidad
